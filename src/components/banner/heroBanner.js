...
const HeroBanner = () => {
  ...
  const [jumlahPupuk, setJumlahPupuk] = useState(0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Ambil data pupuk / alternatif
        const pupukRes = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/api/alternatif`
        );
        const pupuks = pupukRes.data.data || [];
        setJumlahPupuk(pupuks.length);
      } catch (error) {
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              ...
              <p className="text-xl text-gray-600 leading-relaxed">
                Platform inovatif untuk pemilihan dan distribusi pupuk kompos
                organik berkualitas tinggi di desa Anda. Mendukung pertanian
                berkelanjutan dengan teknologi modern.
              </p>
            </div>
