...
const HasilPage = () => {
  ...
  useEffect(() => {
    const fetchResponse = async () => {
      try {
        ...
        const riwayatResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/riwayat/?id_user=${userId}`);
  ...
  const fetchDetailCalculation = async (kodeInput) => {
    ...
    const fetchData = async () => {
      ...
      const [kriteriaRes, alternatifRes, kriteriaJudgmentRes, alternatifJudgmentRes, inputKriteriaRes] = await Promise.all([
