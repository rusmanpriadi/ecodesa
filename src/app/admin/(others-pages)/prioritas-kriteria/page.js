...
const calculateConsistencyMeasure = (matrix, weights, items, itemIdKey = 'id') => {
  ...
  items.forEach(rowItem => {
    ...
    consistencyMeasures[rowItem[itemIdKey]] = cm.toFixed(4);
    totalCM += cm;
  });
    ...
    <div className="min-h-screen bg-gray-50">
                    ...
                    <tr className="bg-gray-100">
                      ...
                      {criteria.map(criterion => (
                        <td key={criterion.id} className="border border-gray-300 px-3 py-2 text-center font-semibold">
                          {criteriaWeights.columnSums?.[criterion.id] || 0}
                        </td>
                      ))}
      ...
      {/* // Tambahkan bagian ini setelah "2. Matriks Normalisasi" dan sebelum "3. Uji Konsistensi" */}
            ...
            <div className="overflow-x-auto">
                ...
                <tbody>
                    {(() => {
                    const { consistencyMeasures } = calculateConsistencyMeasure(criteriaMatrix, criteriaWeights.weights || {}, criteria);
                    return criteria.map(rowCriterion => {
                        const cm = consistencyMeasures[rowCriterion.id];
                        const weight = parseFloat(criteriaWeights.weights?.[rowCriterion.id] || 0);
                        const cmOverWeight = weight > 0 ? (parseFloat(cm) / weight).toFixed(4) : '0.0000';
                    ...
                    <tr className="bg-gray-100">
                    <td className="border border-gray-300 px-3 py-2 font-bold" colSpan={criteria.length + 1}>
                        λmax (Lambda Max)
                    </td>
                    <td className="border border-gray-300 px-3 py-2 text-center font-bold bg-yellow-100">
                        ...
                        return (total / criteria.length).toFixed(4);
                        })()}
                    </td>
                    </tr>
                </tbody>
      ...
      <table className="w-full border border-gray-300">
        <thead>
          <tr className="bg-gray-100">
            ...
            {RI_TABLE.map(item => (
              <th key={item.ordo} className={`border border-gray-300 px-3 py-2 font-semibold text-center ${item.ordo === criteria.length ? 'bg-yellow-100' : ''}`}>
                {item.ordo}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          <tr>
            ...
            {RI_TABLE.map(item => (
              <td key={item.ordo} className={`border border-gray-300 px-3 py-2 text-center ${item.ordo === criteria.length ? 'bg-yellow-100 font-bold' : ''}`}>
                {item.ri.toFixed(2)}
              </td>
            ))}
          </tr>
      ...
      <table className="w-full border border-gray-300">
        ...
        <tbody>
          <tr>
            ...
            <td className="border border-gray-300 px-4 py-3 text-center">{criteria.length}</td>
          ...
          <tr>
            ...
            <td className="border border-gray-300 px-4 py-3 text-gray-600">Berdasarkan ordo matriks {criteria.length}</td>
          </tr>
