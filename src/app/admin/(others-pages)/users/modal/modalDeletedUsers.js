...
export default function ModalDeletedCategori({ onSave, usersData }) {
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(
        `${process.env.NEXT_PUBLIC_API_URL}/api/users/${usersData.id}`
      );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="p-5">
        ...
        <p className="text-sm">Apakah Anda yakin ingin menghapus users ini?</p>
        <DialogFooter>
