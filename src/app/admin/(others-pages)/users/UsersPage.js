...
import { AddUsersModal } from "./modal/modalAddUsers";

import axios from "axios";
import { DataTablePagination } from "@/components/table/data-table-pagination";
import { DataTableToolbar } from "./table/data-table-toolbar";
import ColumnUsers from "./table/columnUsers";
...
const UsersPage = () => {
  ...
  const table = useReactTable({
    data,
    columns: ColumnUsers({ onSave: handleRefreshTable }),
  ...
  return (
    <div className="space-y-4 ">
      <section className="flex items-center justify-between w-full">
        <article>
          <h4 className="text-lg font-semibold">Kelola Users</h4>
          <p className="text-xs text-slate-500 mt-1">
            Make changes to your account here. Click save when you're done.
          </p>
        </article>
        <AddUsersModal  onSave={handleRefreshTable}
